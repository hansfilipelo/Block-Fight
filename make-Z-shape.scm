...
(define (make-Z-shape% color)
  (new 
   (class object%
     ...
     (vector-set! stones 0 (make-stone% 1 7 current-color))
     (vector-set! stones 1 (make-stone% 2 7 current-color))
     (vector-set! stones 2 (make-stone% 2 6 current-color))
     (vector-set! stones 3 (make-stone% 3 6 current-color))
     ...
     (define/public (fall)
       (cond ;checks witch rotation-state it's in
         ...
         ((and (eq? current-rotation 1)
          ...
          (send *player* add-10-score!) ;this gives the player more points for falling from longer distance
          (send this fall))
         (else (send *gameboard* check-top-3)
               (send *player* add-10-score!))))
...
;-----------------------------------ROTATION OF THE SHAPE-------------------------------------------
     ...
     ;in that case move the stones of the shape. Notice that stone nr 3 is fixed.
     (define (rotate-helper)
       (cond
         ;If the shape has rotated 0 or 180 degrees
         ((and (eq? current-rotation 0)
               (send *gameboard* check-position stones (+ (send stone1 get-current-row) 2) (send stone1 get-current-col))
          ...
          (send stone1 move! (+ (send stone1 get-current-row) 2) (send stone1 get-current-col))
         ...
         ;If the shape has rotated 90 or 270 degrees
         ((and (eq? current-rotation 1)
               (send *gameboard* check-position stones (- (send stone1 get-current-row) 2) (send stone1 get-current-col))
               (send *gameboard* check-position stones (- (send stone2 get-current-row) 1) (+ (send stone2 get-current-col) 1))
               (send *gameboard* check-position stones (+ (send stone4 get-current-row) 1) (+ (send stone4 get-current-col) 1)))
     ...
     (define/public (rotate)
       (cond
         ((and (eq? current-rotation 0)
               (eq? (send stone4 get-current-col) 2))
