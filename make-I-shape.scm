...
(define (make-I-shape% color)
  (new 
   (class object%
     ...
     (vector-set! stones 3 (make-stone% 4 6 current-color))
     ...
     (define (rotate-helper)
       (cond 
         ...
         ((and (eq? current-rotation 0)
               ...
               (send *gameboard* check-position stones (- (send stone4 get-current-row) 2) (+ (send stone4 get-current-col) 2)))
          ...
          (send stone4 move! (- (send stone4 get-current-row) 2) (+ (send stone4 get-current-col) 2))
     ...
     (define/public (rotate)
       (cond
         ((eq? (send stone2 get-current-col) 11) 
          ...
          (send this move-left)
          (rotate-helper))
         ((eq? (send stone2 get-current-col) 10) 
          (send this move-left)
          (rotate-helper))
