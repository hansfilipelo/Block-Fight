...
(define (make-J-shape% color)
  (new 
   (class object%
     ...
     (vector-set! stones 2 (make-stone% 3 7 current-color))
...
;-----------------------------------ROTATION OF THE SHAPE-------------------------------------------
     ...
     (define (rotate-helper)
       (cond
         ...
         ((and (eq? current-rotation 0)
               (send *gameboard* check-position stones (+ (send stone1 get-current-row) 1) (send stone1 get-current-col))
         ...
         ((and (send *gameboard* check-position stones (send stone1 get-current-row) (+ (send stone1 get-current-col) 2))
     ...
     (define/public (rotate)
       (cond
         ((and (eq? current-rotation 0)
          ...
          (rotate-helper))
         ((and (eq? current-rotation 2)
               (eq? (send stone3 get-current-col) 2))
          (send this move-right)
          (rotate-helper))
         (else (rotate-helper))
         ))
